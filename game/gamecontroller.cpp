...
GameController::GameController(const std::vector<QString> &info_ls,
    ...
    this->initSidebar();
    static qreal last_calc_fps_time =
    ...
    QObject::connect(frame_timer, &QTimer::timeout, scene, [&] {
        ...
        frame_cnt++;
        if (QTime::currentTime().msecsSinceStartOfDay() - last_calc_fps_time >
            1000) {
    ...
    continue_button =
        new Mypushbutton(view, true, ":/game/assets/continue.png", 150);
    main_menu_button =
        new Mypushbutton(view, true, ":/game/assets/backmain.png", 150);

    help_button = new Mypushbutton(view, false, ":/game/assets/help.png", 150);
    help_button->resize(500, 400);

    QPoint delta = {help_button->rect().width() / 2,
                    help_button->rect().height() / 2};

    help_button->move(view->width() / 2 - delta.x(),
                      view->height() / 2 - delta.y() - 120);
    continue_button->resize(500, 100);
    continue_button->move(view->width() / 2 - delta.x(),
                          view->height() / 2 - delta.y() + 300);

    main_menu_button->resize(500, 100);
    main_menu_button->move(view->width() / 2 - delta.x(),
                           view->height() / 2 - delta.y() + 420);

    connect(continue_button, &Mypushbutton::clicked, continue_button, [&]() {
        QTimer::singleShot(300, this, [&]() { this->gameContinue(); });
    });
    ...
    pauseboard_widgets = std::vector<QWidget *>(
        {continue_button, main_menu_button, help_button});
...
Scenario *GameController::getScenario() const { return scenario; }
...
void GameController::update_game_info() {
    ...
    count_down->setPlainText(
        QString::number(this->scenario->get_last_time() / 100));
    remain_boss_counts->setPlainText(
        QString::number(this->scenario->remain_boss_count()) + " boss left");
}
QGraphicsScene *GameController::getScene() const { return scene; }

bool GameController::isPaused() { return !frame_timer->isActive(); }
...
void GameController::pause() {
    ...
    frame_timer->stop();
    for (auto &pair : timers)
        if (pair.timer->remainingTime() > 0) {
            pair.remainingTime = pair.timer->remainingTime();
            pair.timer->stop();
        }
    this->pause_shield = scene->addRect(scene->sceneRect(), QPen(),
...
void GameController::game_end(const int &is_win) {
    ...
    if (is_win == 1) {
        qDebug() << "win!" << endl;
        Mypushbutton *win_button =
            new Mypushbutton(view, false, ":/backboard/ac.png", 150);
        win_button->resize(500, 100);
        QPoint delta = {win_button->rect().width() / 2,
                        win_button->rect().height() / 2};
        win_button->move(view->width() / 2 - delta.x(),
                         view->height() / 2 - delta.y() - 100);
        win_button->show();
        Mypushbutton *back_button =
            new Mypushbutton(view, true, ":/backboard/back.png", 150);
        back_button->resize(250, 200);
        back_button->move(view->width() / 2 - delta.x() + 60,
                          view->height() / 2 - delta.y() + 50);
        back_button->show();
        ...
        qDebug() << "loss!" << endl;
        Mypushbutton *lose_button =
            new Mypushbutton(view, false, ":/game/assets/die.png", 150);
        lose_button->resize(500, 100);
        QPoint delta = {lose_button->rect().width() / 2,
                        lose_button->rect().height() / 2};
        lose_button->move(view->width() / 2 - delta.x(),
                          view->height() / 2 - delta.y() - 100);
        lose_button->show();
        Mypushbutton *back_button =
            new Mypushbutton(view, true, ":/backboard/back.png", 150);
        ...
        qDebug() << "loss!" << endl;
        Mypushbutton *lose_button =
            new Mypushbutton(view, false, ":/backboard/tle.png", 150);
        lose_button->resize(500, 100);
        QPoint delta = {lose_button->rect().width() / 2,
                        lose_button->rect().height() / 2};
        lose_button->move(view->width() / 2 - delta.x(),
...
void GameController::initSidebar() {
    scene->addRect(QRect(QPoint(850, 0), QPoint(1200, 1000)), QPen(),
                   QBrush(QColor(0, 0, 0, 255)));

    //    auto sidebar_background =
    //    scene->addPixmap(QPixmap(":/game/assets/sidebar.jpg"));
...
KeyboardHandler *GameController::getKbhandler() const { return kbhandler; }
